...
var AddComponent = /** @class */ (function () {
    ...
    function AddComponent(Firestore, afs) {
    ...
    AddComponent.prototype.createTextBoxes = function (n) {
        var _this = this;
        ...
        var node;
        for (var index = 0; index < n; index++) { //make N text boxes
        ...
        jquery__WEBPACK_IMPORTED_MODULE_4__('.input')[0].addEventListener('keyup', function ($event) { return _this.suggestByName($event); });
        ...
        document.addEventListener('keydown', function ($event) {
            ...
            if (filledCount == arr.length) { //if all the boxes had something in them
                ...
                for (var index = 0; index < 5; index++) { //make 5 text boxes
                ...
                for (var x = 0; x < arr1.length; x += 5) {
                    jquery__WEBPACK_IMPORTED_MODULE_4__('.input')[x].addEventListener('keyup', function ($event) { return _this.suggestByName($event); });
    ...
    AddComponent.prototype.suggestByName = function ($event) {
        var _this = this;
        ...
        if ($event.keyCode != 13) { // if they didn't press enter
            ...
            if (/[a-zA-Z0-9-_ ]/.test(inp)) { // if the key pressed was a number letter space hyphen or letter
                ...
                var colRef = this.afs.collection('Inventory');
                var qry = colRef.ref.get()
                    .then(function (snapshot) {
                    snapshot.forEach(function (doc) {
                        if (doc.data().Name.toUpperCase().startsWith(inp)) {
                            _this.temp.push(doc.data());
                        }
                    });
                }).then(function () {
                    if (_this.temp[0] != undefined) {
                        jquery__WEBPACK_IMPORTED_MODULE_4__('.placeholder').val(_this.temp[0].Name);
                    }
                    else {
    ...
    AddComponent.prototype.nextInvID = function () {
        var _this = this;
        var colRef = this.afs.collection('Inventory'); //from the inventory
        var qry = colRef.ref.orderBy('Name', 'asc').get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                if (doc.data().ID != null && _this.id < doc.data().ID) {
                    _this.id = doc.data().ID + 1;
                }
    ...
    AddComponent.prototype.addToInv = function () {
        var _this = this;
        ...
        if (!badEnrtyFlag) { // if all the entries were fine
            ...
            var desc = "";
            var sn = "";
            inputArray.each(function (i, box) {
                //name
                ...
                if (jquery__WEBPACK_IMPORTED_MODULE_4__(box).attr('placeholder').includes('URL') && query != "" && name != "" && typeof qtyEntry == "number") {
                    var colRef = _this.afs.collection('Inventory'); //from the inventory
                    var qry = colRef.ref.orderBy('Name', 'asc').get().then(function (snapshot) {
                        snapshot.forEach(function (doc) {
                    ...
                    if (!updateFlag) {
                        _this.Firestore.inventoryEntry(//enter into db
...
var AppComponent = /** @class */ (function () {
    function AppComponent(Firestore, add) {
        this.Firestore = Firestore;
        this.add = add;
    ...
    AppComponent.prototype.addFirstBoxes = function (e) {
...
/* harmony import */ var ngx_cookie_service__WEBPACK_IMPORTED_MODULE_53__ = __webpack_require__(/*! ngx-cookie-service */ "./node_modules/ngx-cookie-service/ngx-cookie-service.es5.js");
/* harmony import */ var _angular_forms__WEBPACK_IMPORTED_MODULE_54__ = __webpack_require__(/*! @angular/forms */ "./node_modules/@angular/forms/fesm5/forms.js");
...
/* harmony import */ var ng_maphilight__WEBPACK_IMPORTED_MODULE_56__ = __webpack_require__(/*! ng-maphilight */ "./node_modules/ng-maphilight/fesm5/ng-maphilight.js");
...
var DecomissionComponent = /** @class */ (function () {
    function DecomissionComponent(exp, firestore, cookie, db) {
        this.exp = exp;
        this.firestore = firestore;
        this.cookie = cookie;
        this.db = db;
    ...
    DecomissionComponent.prototype.ngOnInit = function () {
        var _this = this;
        var date = new Date();
        var user = this.cookie.get("User");
        this.firestore.getDecom().subscribe(function (data) { _this.items = data; _this.decomSource = new _angular_material__WEBPACK_IMPORTED_MODULE_4__["MatTableDataSource"](data); _this.decomSource.sort = _this.sort; });
    };
    DecomissionComponent.prototype.exportAsCSV = function (table) {
        this.exp.convertToCSV(table);
    };
    DecomissionComponent.prototype.exportAsJSON = function (table) {
        this.exp.convertToJSON(table);
    };
    DecomissionComponent.prototype.applyFilter = function (filterValue) {
        this.decomSource.filter = filterValue.trim().toLowerCase();
    };
    DecomissionComponent.prototype.recomission = function ($event, item) {
        var _this = this;
        var deleted = false;
        var docFound = false;
        ;
        var colRef = this.db.collection('Decomission');
        var invRef = this.db.collection('Inventory');
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                if (doc.data().Name == item.Name && doc.data().Serial == item.Serial && doc.data().Location == item.Location && doc.data().Notes == item.Notes) { //match the doc to the item
                    invRef.ref.get().then(function (snap) {
                        console.log('yaw');
                        snap.forEach(function (invDoc) {
                        ...
                        if (!docFound) {
                            ...
                            if (del) {
                                var colRef_1 = _this.db.collection('Decomission');
                                var qry_1 = colRef_1.ref.get().then(function (snapshot) {
                                    snapshot.forEach(function (doc) {
    ...
    DecomissionComponent.prototype.deleteRow = function ($event, item) {
        var colRef = this.db.collection('Decomission');
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
    ...
    DecomissionComponent.prototype.saveChanges = function () {
        var allboxes = $('.editbox');
        var allboxesArr = $('.editbox').toArray();
        var badEntryFlag = false;
        var container = [];
        allboxes.each(function (v) {
        ...
        if (!badEntryFlag) {
            ...
            var ind = 0;
            for (var x = 0; x < allboxesArr.length; x += 5) {
                var temp = [];
            ...
            var colRef = this.db.collection('Decomission'); //from the inventory
            var qry = colRef.ref.get().then(function (snapshot) {
                snapshot.forEach(function (doc) {
    ...
    DecomissionComponent.ctorParameters = function () { return [
...
var ExportComponent = /** @class */ (function () {
     ...
     */
    ExportComponent.prototype.convertToJSON = function (toExport) {
        var documentArr = []; //temp array
        var colRef = this.db.collection(toExport); // table ref
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                documentArr.push(doc.data()); //add the document to the array
            });
        }).then(function () {
            var data = JSON.stringify(documentArr); //make a Jason string of the doc array
            var uri = 'data:application/json;charset=utf-8,' + encodeURIComponent(data); //some encoding thing idk i found this on the internet
            var exportName = toExport + ".json"; //filename
     ...
     */
    ExportComponent.prototype.convertToCSV = function (toExport) {
        var _this = this;
        var documentArr = [];
        var colRef = this.db.collection(toExport); // table ref
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                var tempData = doc.data(); //make a mutable version of the data to edit
            ...
            _this.exportToCSV(documentArr, toExport); //full send 
        });
    };
    /**
     ...
     */
    ExportComponent.prototype.exportToCSV = function (data, name) {
...
var FirestoreService = /** @class */ (function () {
    function FirestoreService(db, datePipe) {
        this.db = db;
    ...
    FirestoreService.prototype.queryInventory = function (field, value) {
        var invRef = this.db.collection('Inventory');
        var qry = invRef.ref.where(field, '==', value).get()
            .then(function (snapshot) {
            if (snapshot.empty) {
                return;
            }
    ...
    FirestoreService.prototype.nameExistsInTable = function (collection, name) {
        var colRef = this.db.collection(collection);
        var qry = colRef.ref.get()
            .then(function (snapshot) {
            snapshot.forEach(function (doc) {
    ...
    FirestoreService.prototype.nameContainedInTable = function (collection, name) {
     ...
     */
    FirestoreService.prototype.inventoryEntry = function (name, qty, desc, issn, orderUrl) {
    ...
    FirestoreService.prototype.restockEntry = function (name, qty) {
    ...
    FirestoreService.prototype.getNextQueueID = function () {
        var _this = this;
        var makeID = 0;
        var colRef = this.db.collection('Queue');
        var qry = colRef.ref.orderBy('ID', 'desc').get()
            .then(function (snapshot) {
            snapshot.forEach(function (doc) {
                if (doc.data().ID > makeID) {
                    makeID = doc.data().ID + 1;
                    _this.newID = makeID;
                    console.log("newID: ", _this.newID);
                }
            });
            if (makeID = 0) {
                _this.newID = 1;
    ...
    FirestoreService.prototype.getNextInvID = function () {
        var _this = this;
        var makeID = 0;
        var colRef = this.db.collection('Inventory');
        var qry = colRef.ref.orderBy('ID', 'desc').get()
            .then(function (snapshot) {
            snapshot.forEach(function (doc) {
                if (doc.data().ID > makeID) {
    ...
    FirestoreService.prototype.updateRemaining = function () {
        var _this = this;
        this.remainingItems = 0;
        var colRef = this.db.collection('Queue');
        var qry = colRef.ref.orderBy('ID', 'desc').get()
            .then(function (snapshot) {
            snapshot.forEach(function (doc) {
                _this.remainingItems++;
            });
        });
    };
    FirestoreService.prototype.queueEntry = function (name, qty, user, id, serial) {
        this.updateRemaining();
    ...
    FirestoreService.prototype.manageEntry = function (itemName, user, location, notes, quantity, serial, id) {
    ...
    FirestoreService.prototype.decomEntry = function (name, location, user, notes, date) {
    ...
    FirestoreService.prototype.getManage = function () {
        return this.manageItems;
    };
    FirestoreService.prototype.getQueue = function () {
    ...
    FirestoreService.prototype.getInventory = function () {
        //  console.log(this.items);
        return this.items;
    };
    FirestoreService.prototype.getRestock = function () {
        return this.restocks;
    };
    FirestoreService.prototype.getDecom = function () {
        return this.decomItems;
    };
    FirestoreService.prototype.decomission = function () {
...
var InventoryComponent = /** @class */ (function () {
    function InventoryComponent(exp, pin, db, firebaseService, dialog, loc, cookie) {
        this.exp = exp;
        this.pin = pin;
        this.db = db;
    ...
    InventoryComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.firebaseService.getInventory().subscribe(function (data) { _this.items = data; _this.itemsource = new _angular_material__WEBPACK_IMPORTED_MODULE_4__["MatTableDataSource"](data); _this.itemsource.sort = _this.sort; });
    };
    InventoryComponent.prototype.scaleColor = function (qty, last) {
    ...
    InventoryComponent.prototype.invDelete = function () {
        var _this = this;
        var colRef = this.db.collection('Inventory'); // inventory reference
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                _this.db.collection('Inventory').doc(doc.ref.id).delete();
            });
        });
        alert("Cleared Inventory");
    };
    InventoryComponent.prototype.mngDelete = function () {
        var _this = this;
        var colRef = this.db.collection('Manage'); // inventory reference
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                _this.db.collection('Manage').doc(doc.ref.id).delete();
            });
        });
        alert("Cleared Manage");
    };
    InventoryComponent.prototype.queDelete = function () {
        var _this = this;
        var colRef = this.db.collection('Queue'); // inventory reference
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                _this.db.collection('Queue').doc(doc.ref.id).delete();
            });
        });
        alert("Cleared Queue");
    };
    InventoryComponent.prototype.decDelete = function () {
        var _this = this;
        var colRef = this.db.collection('Decomission'); // inventory reference
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
                _this.db.collection('Decomission').doc(doc.ref.id).delete();
            });
        });
        alert("Cleared Decomissioned items");
    };
    InventoryComponent.prototype.massDelete = function () {
        var _this = this;
        var enteredPin = "";
        var dialogConfig = new _angular_material__WEBPACK_IMPORTED_MODULE_4__["MatDialogConfig"](); //options for dialog boxes
        dialogConfig.autoFocus = true;
        dialogConfig.disableClose = true; //so they can't exit
        var dialogRef = this.dialog.open(_pin_keypad_keypad_component__WEBPACK_IMPORTED_MODULE_9__["KeypadComponent"], dialogConfig); //open the keypad 
        dialogRef.afterClosed().subscribe(function (data) {
            enteredPin = data;
            if (_this.pin.pins.includes(enteredPin)) {
    ...
    InventoryComponent.prototype.queueItem = function (e, name, qty, serial) {
        var _this = this;
        var newID = 0;
        var colRef = this.db.collection('Queue');
        var qry = colRef.ref.orderBy('ID', 'asc').get()
            .then(function (snapshot) {
            snapshot.forEach(function (doc) {
            ...
            _this.firebaseService.queueEntry(name, qty, _this.cookie.get("User"), newID, serial);
        });
    };
    InventoryComponent.prototype.decomission = function (e, serial, quantityLeft) {
        var _this = this;
        var qty;
        ...
        if (qty != null && qty != 0 && qty > 0 && qty <= quantityLeft) { // if they entered a number over 0 and its less than or equal to the number remaining
            ...
            var colRef_1 = this.db.collection('Inventory'); // inv ref
            var decRef_1 = this.db.collection('Decomission'); // decom ref
            var qry = colRef_1.ref.get().then(function (snapshot) {
                snapshot.forEach(function (doc) {
                    if (doc.data().Serial == serial) { // find the matching entry
                        colRef_1.doc(doc.id).update({
    ...
    InventoryComponent.prototype.restock = function (e, serial) {
        ...
        if (qty != null && qty != 0 && qty > 0) {
            ...
            var name = a[1];
            var mydate = new Date();
            var colRef_2 = this.db.collection('Inventory');
            var qry = colRef_2.ref.get()
                .then(function (snapshot) {
                snapshot.forEach(function (doc) {
                    if (doc.data().Serial == serial) {
                        colRef_2.doc(doc.id).update({
                            LastRestockQuantity: qty,
                            LastRestock: mydate,
    ...
    InventoryComponent.prototype.deleteRow = function ($event, x) {
        //  console.log(x)
        var allboxesArr = $('.editbox').toArray();
        var colRef = this.db.collection('Inventory'); //from the inventory
        var qry = colRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
    ...
    InventoryComponent.prototype.exportAsCSV = function (table) {
        this.exp.convertToCSV(table);
    };
    InventoryComponent.prototype.exportAsJSON = function (table) {
        this.exp.convertToJSON(table);
    };
    InventoryComponent.prototype.applyFilter = function (filterValue) {
        this.itemsource.filter = filterValue.trim().toLowerCase();
    };
    InventoryComponent.prototype.saveChanges = function () {
        var allboxes = $('.editbox');
        var allboxesArr = $('.editbox').toArray();
        var badEntryFlag = false;
        var container = [];
        ...
        if (!badEntryFlag) {
            console.log('full send boys');
            var ind = 0;
            for (var x = 0; x < allboxesArr.length; x += 6) {
                var temp = [];
                temp.push($(allboxesArr[x]).val().toString());
                temp.push(parseInt($(allboxesArr[x + 1]).toString()));
                temp.push($(allboxesArr[x + 2]).val().toString());
...
var LocationService = /** @class */ (function () {
    function LocationService(map, queue) {
        this.map = map;
        this.queue = queue;
    }
    LocationService.prototype.setCoordinateArray = function () {
    };
    LocationService.prototype.getCoordinate = function (e) {
        var temp = "";
        temp = e.target.id;
...
var ManageComponent = /** @class */ (function () {
    function ManageComponent(exp, db, cookie, firebaseService, dialog, map, dialogRef, data, global) {
        this.exp = exp;
        this.db = db;
        this.cookie = cookie;
        this.firebaseService = firebaseService;
    ...
    ManageComponent.prototype.applyFilter = function (filterValue) {
        this.manageSource.filter = filterValue.trim().toLowerCase();
    };
    ManageComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.firebaseService.getManage().subscribe(function (data) { _this.items = data; _this.manageSource = new _angular_material__WEBPACK_IMPORTED_MODULE_4__["MatTableDataSource"](data); _this.manageSource.sort = _this.sort; });
    };
    ManageComponent.prototype.inventory = function (e, item) {
        var _this = this;
        var description = "";
        var url = "";
        var matchID = "";
        var invRef = this.db.collection('Inventory'); //from the inventory
        var qry2 = invRef.ref.get().then(function (snapshot) {
            snapshot.forEach(function (doc) {
            ...
            if (url != "") {
                var colRef = _this.db.collection('Manage'); //from the inventory
                var qry = colRef.ref.get().then(function (snapshot) {
                    snapshot.forEach(function (doc) {
                        if (item.ID == doc.data().ID) {
                            invRef.ref.get().then(function (invdoc) {
                                invdoc.forEach(function (inventry) {
                                    var n = parseInt(inventry.data().Quantity) + parseInt(item.Quantity);
                                    _this.db.collection('Inventory').doc(matchID).update({
                                        Name: item.Name,
                                        Quantity: n,
    ...
    ManageComponent.prototype.decomission = function (e, serial, quantity, loc, notes) {
        var _this = this;
        var qty;
        qty = prompt("How many are you decomissioning", "1"); //prompt asking how many of the item to send to yeesus
        if (qty <= quantity) {
            if (qty != null && qty != 0 && qty > 0) { // if they entered a number over 0 and its less than or equal to the number remaining
                var colRef_1 = this.db.collection('Manage'); // inv ref
                var decRef_1 = this.db.collection('Decomission'); // decom ref
                var qry = colRef_1.ref.get().then(function (snapshot) {
                    snapshot.forEach(function (doc) {
                        if (doc.data().Serial == serial && serial != null) { // find the matching entry
                            ...
                            colRef_1.doc(doc.id).delete(); //delete the doc
                        }
                    });
                });
            }
    ...
    ManageComponent.prototype.locationPicker = function (e) {
        var _this = this;
        var dialogConfig = new _angular_material__WEBPACK_IMPORTED_MODULE_4__["MatDialogConfig"](); //options for dialog boxes
        dialogConfig.autoFocus = true;
        dialogConfig.hasBackdrop = true;
        var dialogRef = this.dialog.open(_map_map_component__WEBPACK_IMPORTED_MODULE_5__["MapComponent"], dialogConfig); //open the map component
        dialogRef.afterClosed().subscribe(function (data) {
            if (data != null) { //if the componenet sent back some kind of data
                _this.coordArray[e] = data; //e is the index in the coord array that lines up with the queue entry 
            }
        });
    };
    ManageComponent.prototype.exportAsCSV = function (table) {
        this.exp.convertToCSV(table);
    };
    ManageComponent.prototype.exportAsJSON = function (table) {
        this.exp.convertToCSV(table);
    };
    ManageComponent.prototype.saveChanges = function () {
        var allboxes = $('.editbox');
        var allboxesArr = $('.editbox').toArray();
        var badEntryFlag = false;
        var container = [];
        ...
        if (!badEntryFlag) {
            console.log('full send boys');
            var ind = 0;
            for (var x = 0; x < allboxesArr.length; x += 4) {
                var temp = [];
                temp.push($(allboxesArr[x]).val().toString());
                temp.push(parseInt($(allboxesArr[x + 1]).toString()));
                temp.push($(allboxesArr[x + 2]).val().toString());
            ...
            var colRef = this.db.collection('Manage'); //from the inventory
            var qry = colRef.ref.get().then(function (snapshot) {
                snapshot.forEach(function (doc) {
                    for (var x = 0; x < container.length; x++) {
                        if (container[x][0] == doc.data().Name) {
...
var MapComponent = /** @class */ (function () {
    function MapComponent(dialog, dialogRef, data, fb, globals) {
        this.dialog = dialog;
        this.dialogRef = dialogRef;
        this.fb = fb;
    ...
    MapComponent.prototype.locationPicker = function (s) {
        var _this = this;
        ...
        if (s == "offices") { // if they press office button
            ...
            var dialogRef = this.dialog.open(_offices_offices_component__WEBPACK_IMPORTED_MODULE_3__["OfficesComponent"], dialogConfig); //open 
            dialogRef.afterClosed().subscribe(function (data) {
                // console.log(datab)
                return _this.dialogRef.close(data);
            });
        }
        else if (s == "production1") {
            var dialogRef = this.dialog.open(_production1_production1_component__WEBPACK_IMPORTED_MODULE_4__["Production1Component"], dialogConfig); //open 
            dialogRef.afterClosed().subscribe(function (data) {
                // console.log(datab)
                return _this.dialogRef.close(data);
            });
            ...
            var dialogRef = this.dialog.open(_production2_production2_component__WEBPACK_IMPORTED_MODULE_5__["Production2Component"], dialogConfig); //open 
            dialogRef.afterClosed().subscribe(function (data) {
                // console.log(datab)
                return _this.dialogRef.close(data);
            });
            ...
            var dialogRef = this.dialog.open(_shipping_shipping_component__WEBPACK_IMPORTED_MODULE_6__["ShippingComponent"], dialogConfig); //open 
            dialogRef.afterClosed().subscribe(function (data) {
                // console.log(datab)
                return _this.dialogRef.close(data);
            });
    ...
    */
    MapComponent.prototype.getLocation = function (e) {
...
var OfficesComponent = /** @class */ (function () {
    function OfficesComponent(dialogRef) {
        this.dialogRef = dialogRef;
        this.config = {
            fade: true,
            alwaysOn: false,
    ...
    OfficesComponent.prototype.getLocation = function (e) {
        this.coordinate = (e.target.id).toUpperCase();
    };
    OfficesComponent.prototype.closeDialog = function () {
    ...
    OfficesComponent.prototype.onNoClick = function () {
        this.dialogRef.close();
    };
    OfficesComponent.ctorParameters = function () { return [
...
var Production1Component = /** @class */ (function () {
    function Production1Component(dialogRef) {
        this.dialogRef = dialogRef;
        this.config = {
            fade: true,
            alwaysOn: false,
    ...
    Production1Component.prototype.ngOnInit = function () {
    };
    Production1Component.prototype.getLocation = function (e) {
        this.coordinate = (e.target.id).toUpperCase();
    };
    Production1Component.prototype.closeDialog = function () {
        //   this.queue.coordinateArray.push(this.dialogRef.close(this.coordinate));
        return console.log(this.dialogRef.close(this.coordinate));
    };
    Production1Component.prototype.onNoClick = function () {
        this.dialogRef.close();
    };
    Production1Component.ctorParameters = function () { return [
        { type: _angular_material_dialog__WEBPACK_IMPORTED_MODULE_2__["MatDialogRef"] }
    ]; };
...
var Production2Component = /** @class */ (function () {
    function Production2Component(dialogRef) {
        this.dialogRef = dialogRef;
        this.config = {
            fade: true,
            alwaysOn: false,
    ...
    Production2Component.prototype.ngOnInit = function () {
    };
    Production2Component.prototype.getLocation = function (e) {
        this.coordinate = (e.target.id).toUpperCase();
    };
    Production2Component.prototype.closeDialog = function () {
        //   this.queue.coordinateArray.push(this.dialogRef.close(this.coordinate));
        return console.log(this.dialogRef.close(this.coordinate));
    };
    Production2Component.prototype.onNoClick = function () {
        this.dialogRef.close();
    };
    Production2Component.ctorParameters = function () { return [
        { type: _angular_material__WEBPACK_IMPORTED_MODULE_2__["MatDialogRef"] }
    ]; };
...
var ShippingComponent = /** @class */ (function () {
    function ShippingComponent(dialogRef) {
        this.dialogRef = dialogRef;
        this.config = {
            fade: true,
            alwaysOn: false,
    ...
    ShippingComponent.prototype.getLocation = function (e) {
        this.coordinate = (e.target.id).toUpperCase();
    };
    ShippingComponent.prototype.closeDialog = function () {
        //   this.queue.coordinateArray.push(this.dialogRef.close(this.coordinate));
        return console.log(this.dialogRef.close(this.coordinate));
    };
    ShippingComponent.prototype.onNoClick = function () {
        this.dialogRef.close();
    };
...
var KeypadComponent = /** @class */ (function () {
    function KeypadComponent(dialogRef) {
        this.dialogRef = dialogRef;
        this.entered = "";
    }
    KeypadComponent.prototype.ngOnInit = function () {
    };
    KeypadComponent.prototype.addEntry = function (n) {
        if (this.entered.length < 4) {
            this.entered += n.toString();
        }
    };
    KeypadComponent.prototype.back = function () {
        this.entered = this.entered.slice(0, this.entered.length - 1);
    };
    KeypadComponent.prototype.enter = function () {
...
var PinComponent = /** @class */ (function () {
    function PinComponent(queue, dialog, dialogRef, data, cookie) {
        this.queue = queue;
        this.dialog = dialog;
        this.dialogRef = dialogRef;
        this.cookie = cookie;
    ...
    PinComponent.prototype.ngOnInit = function () {
        ...
        if (this.cookie.get("User") == "") { // if the cookie is expired or doesnt exist, prompt keypad
            this.enteredPin = "";
            var dialogConfig = new _angular_material_dialog__WEBPACK_IMPORTED_MODULE_2__["MatDialogConfig"](); //options for dialog boxes
            dialogConfig.autoFocus = true;
            dialogConfig.disableClose = true; //so they can't exit
            var dialogRef = this.dialog.open(_keypad_keypad_component__WEBPACK_IMPORTED_MODULE_4__["KeypadComponent"], dialogConfig); //open the keypad 
            dialogRef.afterClosed().subscribe(function (data) { _this.enteredPin = data; _this.checkPin(); });
        }
     ...
     */
    PinComponent.prototype.checkPin = function () {
        for (var ind = 0; ind < this.pins.length; ind++) {
    ...
    PinComponent.prototype.approvePin = function (user) {
     ...
     */
    PinComponent.prototype.badPin = function () {
        var _this = this;
        this.cookie.deleteAll();
        var dialogConfig = new _angular_material_dialog__WEBPACK_IMPORTED_MODULE_2__["MatDialogConfig"](); //options for dialog boxes
        dialogConfig.autoFocus = true;
        dialogConfig.disableClose = true;
        var dialogRef = this.dialog.open(_keypad_keypad_component__WEBPACK_IMPORTED_MODULE_4__["KeypadComponent"], dialogConfig); //open the map component
        dialogRef.afterClosed().subscribe(function (data) { _this.enteredPin = data; _this.checkPin(); });
...
var QueueComponent = /** @class */ (function () {
    function QueueComponent(exp, db, cookie, dialog, map, dialogRef, data, firebaseService, global) {
        this.exp = exp;
        this.db = db;
        this.cookie = cookie;
        this.dialog = dialog;
    ...
    QueueComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.firebaseService.getQueue().subscribe(function (data) { _this.items = data; _this.queueSource = new _angular_material__WEBPACK_IMPORTED_MODULE_5__["MatTableDataSource"](data); _this.queueSource.sort = _this.sort; });
    ...
    QueueComponent.prototype.exportAsCSV = function (table) {
        this.exp.convertToCSV(table);
    };
    QueueComponent.prototype.exportAsJSON = function (table) {
        this.exp.convertToJSON(table);
    };
    QueueComponent.prototype.applyFilter = function (filterValue) {
     ...
     */
    QueueComponent.prototype.locationPicker = function (e) {
        var _this = this;
        var dialogConfig = new _angular_material_dialog__WEBPACK_IMPORTED_MODULE_3__["MatDialogConfig"](); //options for dialog boxes
        dialogConfig.autoFocus = true;
        dialogConfig.hasBackdrop = true;
        var dialogRef = this.dialog.open(_map_map_component__WEBPACK_IMPORTED_MODULE_4__["MapComponent"], dialogConfig); //open the map component
        dialogRef.afterClosed().subscribe(function (data) {
            if (data != null) { //if the componenet sent back some kind of data
                _this.coordArray[e] = data; //e is the index in the coord array that lines up with the queue entry 
            }
     ...
     */
    QueueComponent.prototype.sendIt = function (name, location, notes, quantity, ID, ind, serial) {
        var _this = this;
        ...
        if (location != null && location != "") { // failsafe null or "" because i dont actually know what the default is, but it has to have a value
            var colRef = this.db.collection('Queue'); //from the queue
            var qry = colRef.ref.orderBy('ID', 'asc').get()
                .then(function (snapshot) {
                snapshot.forEach(function (doc) {
                    if (doc.data().ID == ID) { //find the doc's ID that matches the one passed 
                        _this.notesArray.splice(ind, 1); //remove the item from the array
                        _this.coordArray.splice(ind, 1); /*side note, not checking against serial here because there might be multiple queue'd items for the same serial, but ID is for each queue */
                        _this.db.collection('Queue').doc(doc.ref.id).delete(); // delete the doc from the queue
                        _this.firebaseService.manageEntry(name, _this.cookie.get("User"), location, notes, quantity, serial, doc.data().ID); // and add it to the manage page
                    }
                });
            });
        }
...
var ScanComponent = /** @class */ (function () {
    function ScanComponent(afs, db, cookie, pin) {
        this.afs = afs;
        this.db = db;
        this.cookie = cookie;
        this.pin = pin;
     ...
     */
    ScanComponent.prototype.getParams = function () {
    ...
    ScanComponent.prototype.sendUrlQueue = function () {
        var _this = this;
        this.delay(1500).then(function () {
            if (_this.urlParams.length == 3) { //&& this.db.nameExistsInTable('Inventory', this.urlParams[0])
                while (!_this.pin.approved) {
                    if (_this.db.newID == 0) {
                        _this.db.newID = 1;
                    }
                    _this.db.queueEntry(_this.urlParams[0], _this.urlParams[1], _this.cookie.get("User"), _this.db.newID, _this.urlParams[2]); //the name of the item, the nunmber 
                    break;
                }
            }
        });
    };
    ScanComponent.prototype.delay = function (ms) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
